...
ssh -i ~/.ssh/hetzner_worker "$VPS_USER@$VPS_HOST" "cd /root/clawdbot && if [ -d .git ]; then git reset --hard && git fetch origin && git checkout custom/prod && git pull --rebase origin main; else echo 'No git repo found at /root/clawdbot'; fi"
